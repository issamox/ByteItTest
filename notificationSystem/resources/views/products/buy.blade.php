...
@section('content')
    ...
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl font-bold mb-6 text-center">Buy new quantity of : {{ $product->name }}</h2>

        <!-- Product Create Form -->
        <form action="{{ route('products.buy', $product->id) }}" method="POST" enctype="multipart/form-data">
            @csrf

            <div class="mb-4">
                <label for="quantity" class="block text-sm font-medium text-gray-700">Buy</label>
            ...
            <div class="flex justify-end">
                <button type="submit" class="bg-blue-500 text-white py-2 px-6 rounded-lg hover:bg-blue-600">Buy</button>
            </div>
        </form>
    </div>
@endsection
